...
from body_shape_estimation.yolact.utils.box_utils import decode, jaccard
from body_shape_estimation.yolact.utils import timer
from body_shape_estimation.yolact.utils.box_utils import crop, sanitize_coordinates
from body_shape_estimation.yolact.data.config import cfg, COLORS

pyximport.install(setup_args={"include_dirs": np.get_include()}, reload_support=True)
# from body_shape_estimation.yolact.utils.cython_nms import nms as cnms
...
def draw_img(results, img, visual_top_k=100, visual_thre=0.3, hide_mask=False,
             class_color=False, hide_bbox=False, hide_score=False, fps=None):
    img_gpu = img / 255.0

    # Masks are drawn on the GPU, so don't copy
    masks = results[3][:visual_top_k]
    class_ids, classes, boxes = [x[:visual_top_k].cpu().numpy() for x in results[:3]]

    num_considered = min(visual_top_k, class_ids.shape[0])
    
    for i in range(num_considered):
        if classes[i] < visual_thre:
            num_considered = i
            break
